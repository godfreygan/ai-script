...
func RegisterDefaultNodeHandlers(reg *NodeHandlerRegistry, deps *DefaultDeps) {
	...
	// 以下节点仍走 noop(MVP 阶段):script.split / image.upload / review.submit / human.approve
	reg.Register(NodeScriptSplit, newPassthrough("script.split"))
	reg.Register(NodeImageUpload, newPassthrough("image.upload"))
	reg.Register(NodeReviewSubmit, newPassthrough("review.submit"))
	reg.Register(NodeHumanApprove, newPassthrough("human.approve"))
}
	...
	return func(nc *NodeContext) (map[string]any, error) {
		...
		if resp.Raw != nil {
			if b, ok := resp.Raw["audio_bytes"].([]byte); ok {
				...
				out["audio_bytes_b64"] = base64Bytes(b)
			}
		}
		return out, nil
	}
